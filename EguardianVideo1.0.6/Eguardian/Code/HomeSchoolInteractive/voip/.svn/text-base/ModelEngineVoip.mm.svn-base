...
@synthesize voipAccount; //用户的Voip账号
@synthesize tel;//用户电话
...
- (id)init
{
    if (self = [super init])
    {
        ...
        CCPCallService *service = [[CCPCallService alloc] initWithDelegate:self];
        ...
        sendCount = 0;
        [self performSelector:@selector(testUdpNet) withObject:nil afterDelay:1];
    }
    return self;
}
...
////重置VoipCallService
//- (void)resetVoipCallService{
//    [self.VoipCallService setDelegate:nil];
//    if (self.VoipCallService) {
...
{
//    return [self.VoipCallService sendInstanceMessage:receiver andText:text andAttached:attached andUserdata:@"cloopen"];
...
- (void)onConnected
{
    ...
    if (self.UIDelegate && [self.UIDelegate respondsToSelector:@selector(loginOnConnected)]) {
        //可以证明登录成功
        [self.UIDelegate loginOnConnected];
    }
...
{
    if (reason == ReasonKickedOff){
//        [theAppDelegate printLog:@"你已下线，账号在其他位置登录！"];
        [DELEGATE popPromptViewWithMsg:@"你已下线，账号在其他位置登录！" AndFrame:CGRectMake(0, 380, 320, 30) andDuration:3];
        ...
        if (self.UIDelegate && [self.UIDelegate respondsToSelector:@selector(loginOnConnectError)]) {
            //可以证明登录成功
            [self.UIDelegate loginOnConnectError];
        }
...
//            noti.applicationIconBadgeNumber = [UIApplication sharedApplication].applicationIconBadgeNumber + 1;
            ...
            NSDictionary *infoDic = [NSDictionary dictionaryWithObjectsAndKeys:@"comingCall",KEY_TYPE,callid,KEY_CALLID,callerphone,KEY_CALLERPHONE, callername,KEY_CALLERNAME, [NSString stringWithFormat:@"%d", calltype], KEY_CALL_TYPE, caller, KEY_CALLNUMBER,nil];
...
//获取所有的房间列表
- (void)queryChartroomsOfAppId:(NSString *)appid withKeywords:(NSString *)keywords
{
...
//    NSLog(@"onChatroomeState-------------reason=%d, roomNo=%@", reason, roomNo);
    if (reason == 0 && roomNo.length > 0)
    {
        ...
        if (!isExist)
        {
            [self queryChartroomsOfAppId:appID withKeywords:@""];
        }
    }
...
- (void)onDownloadAttachedWithReason:(NSInteger)reason andFileName:(NSString*) fileName
{
    switch (reason)
    {
        case ESucceed:
        {
            ...
            for (IMMsg *vmsg in self.attachDownArray)
            {
                NSRange range1 = [vmsg.content rangeOfString:@"?fileName="];
                NSString *file = [vmsg.content substringFromIndex:range1.location+range1.length];
                if ([fileN isEqualToString:file])//根据返回的本地文件名找到对应的AppVoiceMsgInfo对象
                {
                    [self confirmInstanceMessageWithMsgId:[NSArray arrayWithObject:vmsg.data]];
                    {
                        vmsg.content = fileName;
                        if ([[fileN pathExtension] isEqualToString:@"amr"])
                        {
                            if (vmsg.duration <=0)
                            {
                                vmsg.duration = [self getVoiceDuration:vmsg.content];
                            }
                        }
                        [self.imDBAccess addNewIM:vmsg];
...
- (void)onReceiveInstanceMessage:(InstanceMsg*) msg
{
    if ([msg isKindOfClass:[IMTextMsg class]])
    {
        ...
        immsg.someone = textmsg.sender;
        immsg.sender = textmsg.sender;
        immsg.userData = textmsg.userData;
//        immsg.loginName = textmsg.loginName;
        immsg.msgtype = 0;
        immsg.isRead = 0;
        immsg.imState = 2;
...
//        NSLog(@"[onReceiveInstanceMessage]text userdata=%@", textmsg.userData);
        if (textmsg.receiver.length>0)
        {
            ...
            {
                immsg.someone = textmsg.receiver;
            }
        }
        ...
        [self.imDBAccess addNewIM:immsg];
        [immsg release];
    ...
    else if ([msg isKindOfClass:[IMAttachedMsg class]])
    {
        ...
        IMMsg *immsg = [[IMMsg alloc] init];
        immsg.data = attachmsg.msgId;
        immsg.content = attachmsg.fileUrl;
        
        immsg.someone = attachmsg.sender;
        immsg.userData = attachmsg.userData;
        if (attachmsg.receiver.length>0)
        {
            NSString *g = [attachmsg.receiver substringToIndex:1];
            if ([g isEqualToString:@"g"])
            {
                immsg.someone = attachmsg.receiver;
            }
        }
...
//        if ([attachmsg.type isEqualToString:@"1"])
//        {
//            downloadInfo.isChunked = YES;
//        }
    ...
    else if([msg isKindOfClass:[IMDismissGroupMsg class]])
    {
        ...
        IMMsg *immsg = [[IMMsg alloc] init];
        immsg.someone = IMGROUP_NOTIFY_MESSAGE_SOMEONE;
        immsg.data = instanceMsg.groupId;
        immsg.isRead = 0;
        ...
        if (self.UIDelegate && [self.UIDelegate respondsToSelector:@selector(responseMessageStatus:callNumber:data:)])
        {
            [self.UIDelegate responseMessageStatus:EMessageStatus_Received callNumber:instanceMsg.groupId data:@"群组解散"];
        }
    }
    else if([msg isKindOfClass:[IMInviterMsg class]])
    {
        ...
        immsg.imState = 2;
        immsg.sender = instanceMsg.admin;
        immsg.content = instanceMsg.declared;
        immsg.msgtype = [instanceMsg.confirm integerValue];
        ...
        if (self.UIDelegate && [self.UIDelegate respondsToSelector:@selector(responseMessageStatus:callNumber:data:)])
        {
            [self.UIDelegate responseMessageStatus:EMessageStatus_Received callNumber:instanceMsg.groupId data:@"有群组邀请加入"];
        }
    }
    else if([msg isKindOfClass:[IMProposerMsg class]])
    {
        ...
        immsg.imState = 0;
        immsg.content = instanceMsg.declared;
        immsg.sender = instanceMsg.proposer;
        immsg.msgtype = 0;
        ...
        if (self.UIDelegate && [self.UIDelegate respondsToSelector:@selector(responseMessageStatus:callNumber:data:)])
        {
            [self.UIDelegate responseMessageStatus:EMessageStatus_Received callNumber:instanceMsg.groupId data:@"有人申请加入群组"];
        }
    }
    else if([msg isKindOfClass:[IMQuitGroupMsg class]])
    {
        ...
        immsg.imState = 4;
        immsg.sender = instanceMsg.member;
        immsg.content = @"退出群组";
        ...
        if (self.UIDelegate && [self.UIDelegate respondsToSelector:@selector(responseMessageStatus:callNumber:data:)])
        {
            [self.UIDelegate responseMessageStatus:EMessageStatus_Received callNumber:instanceMsg.groupId data:@"有人退出群组"];
        }
    }
    else if([msg isKindOfClass:[IMRemoveMemberMsg class]])
    {
        ...
        if (self.UIDelegate && [self.UIDelegate respondsToSelector:@selector(responseMessageStatus:callNumber:data:)])
        {
            [self.UIDelegate responseMessageStatus:EMessageStatus_Received callNumber:instanceMsg.groupId data:@"被移除群组"];
        }
    }
    else if([msg isKindOfClass:[IMReplyJoinGroupMsg class]])
    {
        ...
        immsg.content = instanceMsg.confirm==0? @"通过你加入群组":@"拒绝你加入群组";
        immsg.sender = instanceMsg.admin;
        immsg.msgtype = [instanceMsg.confirm integerValue];
        ...
        if (self.UIDelegate && [self.UIDelegate respondsToSelector:@selector(responseMessageStatus:callNumber:data:)])
        {
            [self.UIDelegate responseMessageStatus:EMessageStatus_Received callNumber:instanceMsg.groupId data:@"申请加入群组消息回复"];
        }
    }
...
-(void)postRespondsEvent:(ASIHTTPRequest*)request andElement:(GDataXMLElement*)element andReason:(NSInteger) reason andData:(NSMutableDictionary*) data
{
    ...
    else if (request.requestType == ERequestType_Group_CreateGroup)
    {
        ...
        //默认创建群
        if (reason == 0) {
            NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:NotificationCreateGroupInstance,NotificationKey,NotificationCreateGroupName,NotificationSecondKey, nil];
            [[NSNotificationCenter defaultCenter] postNotificationName: NotificationCreateGroupInstance object:[data objectForKey:KEY_RESPONSE_USERDATA] userInfo: userInfo];
        }
    ...
    else if (request.requestType == ERequestType_Member_QueryGroup)
    {
        ...
        //默认创建群
        if (reason == 0) {
            NSDictionary *userInfo = [NSDictionary dictionaryWithObjectsAndKeys:NotificationCreateGroupInstance,NotificationKey,NotificationGroupList,NotificationSecondKey, nil];
            [[NSNotificationCenter defaultCenter] postNotificationName: NotificationCreateGroupInstance object:[data objectForKey:KEY_RESPONSE_USERDATA] userInfo: userInfo];
        }
    }
 ...
 */
- (void) createGroupWithName:(NSString*) name andType:(NSString*) type andDeclared:(NSString*) declared andPermission:(NSInteger) permission
{
    NSString *timestamp = [self getTimestamp];
    ...
    {
        xmlBody = xmlBody = [NSString stringWithFormat:@"<?xml version='1.0' encoding='UTF8'?><Request><name>%@</name><type>%@</type><declared></declared><permission>%d</permission></Request>", name, type,permission];
    }
    else
        xmlBody = xmlBody = [NSString stringWithFormat:@"<?xml version='1.0' encoding='UTF8'?><Request><name>%@</name><type>%@</type><declared>%@</declared><permission>%d</permission></Request>", name, type,declared,permission];
    [request appendPostData:[xmlBody dataUsingEncoding:NSUTF8StringEncoding]];
    [request setDelegate:self];
    [request setValidatesSecureCertificate:NO];
    [request startAsynchronous];
...
- (void) modifyGroupCard:(IMGruopCard *)gruopCard
{
    ...
    if ([gruopCard.voipAccount length] > 0)
    {
        voip = [NSString stringWithFormat:@"<voipAccount>%@</voipAccount>",gruopCard.voipAccount];
    }
...
{    
    ...
    if (groupsInfo.count > 0)
    {
        ...
        if (groupsArray.count > 0)
        {
            ...
            {
                IMGroup* imGroup = [[IMGroup alloc] init];
                ...
                {
                    GDataXMLElement *valueElement = (GDataXMLElement*)[typeArray objectAtIndex:0];
                    imGroup.type = valueElement.stringValue;
                }
