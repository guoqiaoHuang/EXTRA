...
- (void)viewDidLoad
{
    ...
    tempHandFreeButton.frame = CGRectMake(23.5f+91.0f, 0.0f, 91.0f, 43.0f);
    ...
    tempMuteButton.frame = CGRectMake(23.5f+91.0f*2, 0.0f, 91.0f, 43.0f);
...
-(void)responseVoipManagerStatus:(ECallStatusResult)event callID:(NSString*)callid data:(NSString *)data
{
    ...
    switch (event)
    {
        ...
        case ECallStatus_Failed:
        {
            ...
            else if( data.intValue == 410 )
            {
                self.realTimeStatusLabel.text = @"呼叫失败，请查询余额";
            }
            else
            {
