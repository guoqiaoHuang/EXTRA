...
@property (nonatomic, retain) UILabel *mwText;
// Private Methods
...
//        self.hidesBottomBarWhenPushed = YES;
        //[theAppDelegate.customTabBar setHidden:YES withAnimation:YES];
...
//    _toolbar.barStyle = UIBarStyleBlackTranslucent;
    ...
    _mwText = [[UILabel alloc] initWithFrame:CGRectMake(30, 240, 300, 50)];
    _mwText.hidden = YES;
    [_pagingScrollView addSubview:_mwText];
    _mwText.font = [UIFont systemFontOfSize:17];
    _mwText.backgroundColor = [UIColor clearColor];
    _mwText.textColor = [UIColor whiteColor];
...
//    }
    ...
    rightBarButton = [CommonTools navigationItemNewBtnInitWithTitle:@"保存" target:self action:@selector(savePhoto)];
    UIBarButtonItem* rightBarBtn = [[UIBarButtonItem alloc] initWithCustomView:rightBarButton];
    self.navigationItem.rightBarButtonItem = rightBarBtn;
    [rightBarBtn release];
    UIBarButtonItem *backbutton = [[UIBarButtonItem alloc] initWithCustomView:[CommonTools navigationItemNewBtnInitWithTitle:@"返回" target:self action:@selector(doneButtonPressed:)]];
...
//    {
//        [self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"navigation_column.png"] forBarMetrics:0];        
//    }
...
//            [[UIApplication sharedApplication] setStatusBarHidden:hidden animated:animated];
            if ([[UIApplication sharedApplication] respondsToSelector:@selector(setStatusBarHidden:withAnimation:)]) {
                ...
                [sharedApp setStatusBarHidden:NO animated:animated];
            }
...
-(void)hideActiveViewControls
{
    MWZoomingScrollView *zoom = [self pageDisplayedAtIndex:0];
    [zoom set_spinnerHide];
    [self cancelControlHiding];
    [self setControlsHidden:NO animated:YES permanent:YES];
    [self releaseAllUnderlyingPhotos];
    _mwText.text = @"未能下载图片，原图片可能已经删除！";
    _mwText.hidden = NO;
    [_pagingScrollView bringSubviewToFront:_mwText];
//    [self performSelector:@selector(goRoot) withObject:nil afterDelay:1.];
