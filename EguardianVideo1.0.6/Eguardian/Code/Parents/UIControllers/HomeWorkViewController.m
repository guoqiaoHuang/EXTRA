...
-(void)selectDateFinish
{
    ...
    NSString *tempURL = [ConfigManager getHomeWorkWithString:[NSString stringWithDate:self.date]];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    {
        adScrollView = [[ADScrollView alloc] initWithName:@"homework" delegate:self];

        self.adScrollView.frame = CGRectMake(0, self.view.frame.size.height-navh-self.adScrollView.frame.size.height, self.adScrollView.frame.size.width, self.adScrollView.frame.size.height);
...
- (void)netWorkUpdateReadNumber{
    id typeID = @"ZY";
    id detailID = [readNumberDic objectJudgeFullForKey:@"zy_ids"];
    id countID = [readNumberDic objectJudgeFullForKey:@"zuoyeweidu"];
    if (detailID && countID ) {
            ...
            [NSURLConnection sendAsynchronousRequest:request
                                   ...
                                   completionHandler:^(NSURLResponse *response, NSData *temdata, NSError *error) {
                                       ...
                                       if (error)
                                       {
                                           ...
                                           //这里没做判断ok
                                           [readNumberDic setObject:@"0" forKey:@"zuoyeweidu"];
                                       }
                                   }];
...
-(UITableViewCell *)tableView:(UITableView *)atableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *tempStr = [NSString stringWithFormat:@"%@  %@",[tempDic objectForKey:@"subject"],[tempDic objectForKey:@"zuoye"] ];
    ptable.text = tempStr;
...
{
    HomeWorkData *cdata = [[HomeWorkData alloc] initWithDictionary: [JSONProcess JSONProcess:tempData]];
