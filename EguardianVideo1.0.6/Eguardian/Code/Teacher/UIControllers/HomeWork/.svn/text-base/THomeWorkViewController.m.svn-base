...
@interface THomeWorkViewController ()

@end

@implementation THomeWorkViewController
...
-(void)selectDateFinish
{
    ...
    NSString *tempURL = [ConfigManager getTHomeWorkWithString:[NSString stringWithDate:self.date]];
...
-(UITableViewCell *)tableView:(UITableView *)atableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ConentTable.text = [NSString stringWithFormat:@"%@",[tempDic objectForKey:@"content"] ];
    
    UILabel *subjectTable = (UILabel *)[cell viewWithTag:91];
    subjectTable.text = [NSString stringWithFormat:@"%@",[tempDic objectForKey:@"subject"] ];
...
{    
    TDetailController *dc = [[TDetailController alloc] initWithTitle:@"作业详情" data:[self.info.list objectAtIndex:indexPath.row]];
...
- (void) N_didFinsh:(id)tempData dataMessage:(NSString *) msg
{
    ...
    THomeWorkData *cdata = [[THomeWorkData alloc] initWithDictionary: [JSONProcess JSONProcess:tempData]];
