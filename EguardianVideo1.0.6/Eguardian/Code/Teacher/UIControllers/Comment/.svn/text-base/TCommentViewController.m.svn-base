...
-(void)selectDateFinish
{
    ...
    NSString *tempURL = [ConfigManager getTCommentWithDate:[NSString stringWithDate:self.date]];
...
-(UITableViewCell *)tableView:(UITableView *)atableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ConentTable.text = [NSString stringWithFormat:@"%@",[tempDic objectForKey:@"pingjia"] ];
    
    UILabel *subjectTable = (UILabel *)[cell viewWithTag:91];
    subjectTable.text = [NSString stringWithFormat:@"%@",[tempDic objectForKey:@"sname"] ];
...
{
    TCommentDetailController *dc = [[TCommentDetailController alloc] initWithTitle:@"详情" data:[self.infoArray objectAtIndex:indexPath.row]];
